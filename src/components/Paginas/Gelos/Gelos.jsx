...
const Gelos = () => {
  ...
  useEffect(() => {
    const fetchProdutos = async () => {
      try {
        ...
        console.error('Erro ao carregar os gelos:', error);
      } finally {
        setCarregando(false);
      }
    };
  ...
  return (
    <div className="gelo-page-container">
      <header className="gelo-header">
        <div className="gelo-header-content">
          <h1>Gelos</h1>
          <p>PARA MANTER SUAS BEBIDAS NA TEMPERATURA IDEAL</p>
        </div>
      </header>

      <main className="gelo-main-content">
        ...
        {carregando ? (
          ...
          <div className="gelo-grid">
            {produtosFiltrados.map((produto) => (
              <div className="gelo-card" key={produto.id}>
                <div className="gelo-card-image-container">
                    <img src={produto.imagem_url} alt={produto.name} />
                </div>
                <div className="gelo-card-content">
                    <h3>{produto.name}</h3>
                    <p className="gelo-preco">
          ...
          <p className="nenhum-produto">Nenhum gelo encontrado.</p>
        )}
      </main>
