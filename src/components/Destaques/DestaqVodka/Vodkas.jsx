...
import '../DestaqVinho/DestaqueVinho.css'; // Reutilizando o CSS do Vinho
...
const Vodka = () => {
    ...
    const fetchData = async () => {
      try {
        const { data, error } = await supabase
          .from('produtos')
          .select('*')
          .eq('category', 'VODKAS') // Filtra produtos da categoria 'VODKA'
  ...
  return (
    <div className="vinho-container">
      <h1>Destaques de Vodkas</h1>
      {carregando ? (
        <p>Carregando produtos...</p>
      ) : (
        <Slider {...settings}>
          {produtos.map((produto, index) => (
            <div className="vinho-produto-card" key={index}>
              <div className="vinho-produto-imagem-container">
                <img className="vinho-produto-imagem" src={produto.imagem_url} alt={produto.nome} />
              </div>
              <h3>{produto.name}</h3>
      ...
      <button className="vinho-ver-mais-button" onClick={() => navigate('/vodkas')}>
        Ver Mais
      </button>
    </div>
  );
